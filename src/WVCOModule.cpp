...
void WVCOWidget::appendContextMenu(Menu* menu) {
    ...
    item = new SqMenuItem_BooleanParam2(module, Comp::DO_FM_NOT_PM_PARAM);
    item->text = "Use FM";
    menu->addChild(item);

    item = new SqMenuItem_BooleanParam2(module, Comp::SET_F_TO_ZERO_PARAM);
    item->text = "Freq Zero";
    menu->addChild(item);

    item = new SqMenuItem_BooleanParam2(module, Comp::OVERSAMPLE_FM);
    item->text = "Oversample FM";
    menu->addChild(item);
}
